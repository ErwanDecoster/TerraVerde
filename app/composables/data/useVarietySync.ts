...
export const useVarietySync = () => {
   ...
   */
  const syncVarietyInPlants = (
    plants: Ref<PlantData[]>,
    updatedVariety: VarietyData,
  ) => {
    const plantsToUpdate = plants.value.filter(
      (plant) => plant.variety_id.toString() === updatedVariety.id.toString(),
    );
   ...
   */
  const removeVarietyFromPlants = (
    plants: Ref<PlantData[]>,
    deletedVarietyId: string,
  ) => {
    const remainingPlants = plants.value.filter(
  ...
  const addVarietyToList = (
    varieties: Ref<VarietyData[]>,
    newVariety: VarietyData,
  ) => {
    varieties.value.unshift(newVariety);
  };

  const syncVarietyInList = (
    varieties: Ref<VarietyData[]>,
    updatedVariety: VarietyData,
  ) => {
    const index = varieties.value.findIndex(
      (v) => v.id.toString() === updatedVariety.id.toString(),
    );
    if (index !== -1) {
  ...
  const removeVarietyFromList = (
    varieties: Ref<VarietyData[]>,
    deletedVarietyId: string,
  ) => {
    varieties.value = varieties.value.filter(
      (v) => v.id.toString() !== deletedVarietyId.toString(),
    );
  };
